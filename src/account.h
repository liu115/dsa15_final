...
{
public:
	Account(const std::string& s1, const std::string& s2): _id(s1), _passwd(s2), _deposit(0) {}
	~Account() {}
	...
	void show() {
		...
		for (int i=0; i<_log.size(); ++i) {
			std::cout << ((_log[i].isFrom)?"From":"To") << " " << _log[i].id << " " << _log[i].amount << std::endl;
		}
	}
...
private:
	...
	struct logNode
	{
		...
		int amount;
		bool isFrom;
		logNode(const bool b, const int i, const std::string s): isFrom(b), amount(i), id(s) {}
	};
	std::string _id;
	std::string _passwd;
	int _deposit;
